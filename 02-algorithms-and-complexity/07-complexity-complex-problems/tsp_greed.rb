...
def nearest_possible_neighbor(current_city)
  ...
  # puts "1. nearest_neighbor = #{nearest_neighbor.city.name}"

  # iterate thru neighbors to find the closest
  neighbor_cities.each do |current_neighbor|
    puts "current neighbor being evaluated:  #{current_neighbor.city.name}  #{current_neighbor.city.visited}"
    if current_neighbor.city.visited == false && current_neighbor.distance < nearest_neighbor.distance
      ...
      puts "------------winning so far:  #{current_neighbor.city.name}"
    end
  end
  nearest_neighbor.city.visited = true   # mark it visited
  puts "-----------returning nearest_neighbor #{nearest_neighbor.city.name} #{nearest_neighbor.city.visited}"
...
c3.neighbors = [Neighbor.new(c1, 566), Neighbor.new(c2, 783), Neighbor.new(c0, 2473), Neighbor.new(c4, 1275), Neighbor.new(c5, 417) ]
c4.neighbors = [ Neighbor.new(c1,  1276), Neighbor.new(c2, 1980), Neighbor.new(c3, 1275), Neighbor.new(c0, 1276), Neighbor.new(c5, 1694) ]
